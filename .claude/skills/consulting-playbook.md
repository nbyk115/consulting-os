...
## マスク式 2 原則（2026-05-08 統合 + アプデ、references/ 分離）

`strategy-lead` + `client-success` + `marketing-director` 連携の横断 2 原則。前節 §11「致命傷を避ける 2 原則」が立ち上げ / 撤退の上流判断、本節は日常運用 + 資産化判断を担う。

「原則 1: インセンティブ設計（罰則 / 報酬対称配置）」: 罰則系仕組みを導入する全提案で「報酬系仕組み」との対称配置を必須化（罰則のみの片肺設計は漸進バイアス + Goodhart 化を加速）。適用 3 軸 = 罰則 / 報酬の対称配置 / 失敗コストの上限管理 / 漸進 vs 大胆の比率宣言。

「原則 2: 評判資本主義（2026-05-08 アプデ）」: 真の資産は現金でなく評判 / 知識。市場が評判を資本へ自動変換する事象（サム・アルトマン無制限リソース / ミラ・ムラティ製品ゼロで $2B / Adam Neumann 数億ドル即調達）。KPI / 報酬 / 投資判断で「現金 = 遅行指標」と「評判 / 知識 / 実証 ROI = 真の資産」を区別、後者最大化を強制。CLAUDE.md ハードルール 1「真の 100 原則」と完全整合（形式達成度 100/100 vs 実クライアント ROI 実証）。

詳細: [`references/consulting-playbook-musk-incentive.md`](references/consulting-playbook-musk-incentive.md)
...
ハードルール 13「SKILL.md 500 行以下、超過時 references/ 分離」遵守のため、以下のセクションを分離（2026-05-05 PR #49）:
...
- 4 思想兼務体制の運用順序
- マスク式 2 原則: インセンティブ設計 + 評判資本主義（2026-05-08 統合 + アプデ）

詳細:
- [`.claude/skills/references/consulting-playbook-thoughts.md`](references/consulting-playbook-thoughts.md)
- [`.claude/skills/references/consulting-playbook-musk-incentive.md`](references/consulting-playbook-musk-incentive.md)
