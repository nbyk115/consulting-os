# N.Y.CRAFT 関根さん案件：OEM第1号 戦略＆提案パッケージ（v3）

コンサルOS の OEM販売における第1リファレンス案件として、
N.Y.CRAFT（代表：関根 優氏）への提案・契約・運用設計をまとめたドキュメント群。

> 2026-05-02 提案 PDF クライアント送付済。
> 本案件は ConsultingOS の OEM 第 1 リファレンス事例としてポートフォリオ化済：
> [`examples/n-y-craft-consulting-os-oem/`](../../examples/n-y-craft-consulting-os-oem/)（v1→v7 イテレーション記録・規律違反是正・再現手順を含む）

> v3 更新（2026-04）：コンサルOS本体が 30エージェント × 19スキル × 6部門 + 反証モード + 業界プレイブック + 思想的基盤組込み に進化したため、提案・価格・スライドを練り直し。さらに既存業務委託（月5万・壁打ち）で深度インタビュー不要の前提で構築工数 110h → 約 68h に圧縮、プロジェクトフィー 100万 → 50万、Light 75万 → 30万 に引下げ。
