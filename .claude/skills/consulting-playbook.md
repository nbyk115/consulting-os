...
## アルトマン式ソロ創業戦略 5 原則（2026-05-09 統合、references/ 分離）

サム・アルトマン公開対談「AI エージェント時代のソロ創業者戦略」(INFERENCE)。ソロ創業 / スモビジ案件の AI エージェント時代戦略。マスク式 4 原則と並列、5 原則 = AI 1 人チーム実現 / エージェント管理 CEO スキル / SaaS 役割変容 (垂直特化) / GDP インパクト評価 / 実践 5 件 (Codex Desktop + FDE + compute power)。

詳細: [`references/consulting-playbook-altman-solopreneur.md`](references/consulting-playbook-altman-solopreneur.md)
...
- 4 思想兼務体制の運用順序
- マスク式 4 原則: インセンティブ設計 + 評判資本主義 + Unkind Truth + アルゴリズム実行（2026-05-08 統合 + 拡張）
