...
### 2.6 エージェント失敗時フォールバック原則（2026-05-07 ユーザー指摘「コンサル OS 起動しろっていったよな」物理化）

エージェント並列起動が失敗（rate limit / timeout / tool error / レスポンス空）した場合、orchestrator (assistant) は以下のいずれかを選択する。「自分で書く」への即フォールバックは構造的怠慢として禁止:

許可された対応:
- 対応 A 即時リトライ: rate limit なら待機後リトライ、tool error なら別ツール経由でリトライ
- 対応 B 構成変更: 失敗エージェントを別エージェント（同等機能）に切替、または並列数を減らして単独起動
- 対応 C 分割再起動: タスク粒度を細分化して 1 エージェント = 1 サブタスクで再起動
- 対応 D ユーザー判断仰ぎ: 上記 A/B/C 全て不可の場合のみ、失敗事象 + 影響範囲 + 推奨選択肢をユーザーに提示

**NEVER**: 「エージェント失敗 → 自分で WebSearch + 自分で書く」の即フォールバック。これは「assistant が ConsultingOS のフリをして書いた」だけで、ConsultingOS 起動の指示に対する役割放棄。

検知パターン（自己モニタリング義務）:
- エージェント起動失敗を検知したら、即「対応 A → B → C → D」を順に検討した記録を応答内に残す
- 「rate limit だったので自分で WebSearch しました」という記述は構造的怠慢の兆候 → 次回から対応 A-D で再構成

学習根拠: 2026-05-07 Incendium 案件 v1 で、エージェント 2 並列起動 (competitive-analyst + market-researcher) が rate limit 失敗 → 即「自分で WebSearch + 自分で書く」にフォールバック → AI ネイティブ国内勢を完全見落とし → ユーザー指摘 3 件で発覚（evolution-log 2026-05-07 エントリ）。
...
### 4.4 サーチクエリ網羅性ゲート（2026-05-07 物理化）

競合分析 / 市場調査 / 業界調査の全タスクで、以下のサーチクエリ網羅性を機械的に確認する。1 つでも未実施なら再調査必須:

#### 4.4.1 必須サーチクエリ（全調査共通）
- `<カテゴリ> 大手代理店 AI ソリューション <year>` （電通 / 博報堂 / サイバーエージェント / 大手 SI の AI 戦略確認）
- `<カテゴリ> AI ネイティブ <region> <year>` （AI ファースト新興プレイヤー確認）
- `<カテゴリ> スタートアップ <region> <year>` （新興プレイヤー確認）
- `<老舗ベンダー> AI <year> アップデート` （老舗の AI ネイティブ化進行確認）

#### 4.4.2 AI プロダクト調査の追加必須クエリ
- `<競合カテゴリ> 生成AI エージェント <year>` （AI エージェント路線の検出）
- `<業界> 生成AI ツール 日本 <year>` （日本特化 AI ツール検出）
- `<プラットフォーム> AI ネイティブ <year>` （プラットフォーム純正 AI 検出）

#### 4.4.3 隣接角度ゲート 3 問（自己反証 Step 1 の必須サブステップ）
反証チェック Step 1（自己反証）の冒頭で以下 3 問に YES/NO で回答:
1. 同類の AI ネイティブを検索したか?
2. 大手代理店 (電通 / 博報堂 / サイバー / 大手 SI) の同等ソリューションを検索したか?
3. 海外同等製品の日本展開状況を検索したか?

3 問全て YES でない場合、Step 1 不合格として再調査必須。NO のまま納品物を出力することは Hard Rule 1 (FACT 偽装) 違反として扱う。
