...
## マスク式インセンティブ設計原則（2026-05-08 統合、references/ 分離）

`strategy-lead` + `client-success` + `marketing-director` 連携の横断原則。罰則系仕組みを導入する全提案で「報酬系仕組み」との対称配置を必須化（罰則のみの片肺設計は漸進バイアス + Goodhart 化を加速）。適用 3 軸 = 罰則 / 報酬の対称配置 / 失敗コストの上限管理 / 漸進 vs 大胆の比率宣言。前節 §11「致命傷を避ける 2 原則」が立ち上げ / 撤退の上流判断、本節は日常運用での大胆挑戦の継続条件を担う。
