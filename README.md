...
# CLI 経由
consulting-os --agent strategy-lead --request "N.Y.CRAFT の OEM 戦略を 3 軸で評価"

# Python から
python -c '
from consulting_os import invoke
result = invoke(
    agent="strategy-lead",
...
print(result.output)
print("discipline_pass:", result.discipline_pass)
print("violations:", result.violations)
'
```

Runtime layer は Anthropic SDK を底に置き、ConsultingOS が「反証チェック自動付与」「FACT/INFERENCE/SPECULATION 検証」「規律違反の機械検知」を thin wrapper で enforce する。詳細: [`consulting_os/`](consulting_os/) + 17 件の pytest 全 PASS。
...
graph TB
    ...
    subgraph Departments["6 部門・27 エージェント"]
    ...
    subgraph Skills["33 スキル（直下 24 + サブディレクトリ 9）"]
...
| evolution-log | 違反学習 多数記録（実態 33 件以上、再評価カレンダー 48 件以上） |
| 再評価カレンダー | 7 項目（自動通知） |
...
Anthropic が「Claude Code でエージェントを作れる」ようにした。ConsultingOS はその上で「規律ある会社として動く」ようにした。反証ゲート・物理ブロック・6 部門 27 エージェント連動・自己進化ログを実装済みで、Anthropic 公式機能との完全互換を保ちながら、「危険な AI ツール」から「出荷可能な AI 組織 OS」に昇格させる規律レイヤーである。
...
| Skills（SKILL.md 形式の専門能力テンプレート） | 33 スキル + 6 部門 27 エージェントへの配布ルール整備。SKILL.md 500 行以下制約で形骸化を防止。追加は削除とセット（Boris #3 ruthlessly edit） | `.claude/skills/` + `evolution-log.md` 再評価カレンダー | FACT |
...
軸 3: 6 部門 27 エージェント統合 + 4 兼務体制（連動推奨）
...
| 自前 LLM 構築 / 社内 AI 内製 | 機密対応・カスタマイズ | 構築コスト 0（既存 Claude Code に上乗せ）、6 部門 27 エージェントは内製より大幅に短い期間で到達（個人見立て、SPECULATION、実案件比較データ未取得）| 導入速度 × コスト構造 |
| 大手コンサルファーム | 人的リソース・実績 | 属人化必至の人的モデル vs 再現可能な構造モデル。佐藤裕介流「構造・再現性で売る」に完全対応 | 再現性・スケーラビリティ |

### Phase 6.5 需要必然性ロジック（ConsultingOS の存在意義）

> INFERENCE: McKinsey TNW Insights 2026 報道 + ユーザー考察 + Simon Willison 2026-05-06 + 4 スライド画像（出典源 INFERENCE）の統合。

業界課題と ConsultingOS の物理的解決:

| 業界課題（出典） | ConsultingOS 解決構造 |
|---|---|
| 95% の企業が AI から ROI を得られていない、56% が「まだ何も得られていない」（McKinsey） | 27 エージェント + 11 hook + score-os-health.sh で 100/100 を機械検証可能（INFERENCE: 形式達成度、PR Y トリプルチェックで採点基準脆弱性 4 件発見済、Phase 4 採点ロジック根本改善予定）「10:80:10 オーケストレーション」のオーケストレーター部分を構造化 |
| AI 節約時間の 40% が出力確認・修正に消失（McKinsey） | reality-check.sh + stop-validator.sh + score-os-health.sh + 反証 Step 3 実測必須化で確認・修正の構造化 |
| AI ブラックボックス化への懸念（Simon Willison） | ハードルール 1 完了系断言には実測コマンド + 結果添付必須、ブラックボックスを物理的に防止 |
| AI 導入の死の谷 5 課題（4 スライド画像） | ai-readiness-assessment §1-3 で 5 軸診断と完全マッピング、死の谷越えの構造化 |
| 上流レイヤー人材 4 コア機能（評価目利き / 課題翻訳 / 実装定着 / 経営技術橋渡し） | competitive-analyst / consulting-playbook / engineering-playbook / strategy-lead が各機能を担当、6 部門 27 エージェントで体系化 |
| 主流は「少人数 + AI + 外部人材を束ねる会社」（再組織化ビジョン） | assistant = orchestrator（少人数リーダー）+ 27 エージェント（AI 内部チーム）+ 6 部門スキル（外部人材相当の専門知見） |

ターゲット ICP（ICP.md 準拠）:
- 中堅企業 CMO / CTO / 経営者（AI Readiness 診断ニーズ）
- 個人 PE 投資家（水野氏 OEM 経路、投資先 DD + ポートフォリオモニタリング）
- 中小代理店（advertising-agency 成果報酬モデル転換ニーズ）
- 投資家向けピッチ資料用途（monopoly-positioning Phase 3 革新フェーズ）

ConsultingOS の競争優位は「AI ブラックボックス化を構造で防ぐ + 個人がプロ AI 駆使を再現できる + 6 部門統合で死の谷を越える」= AI 駆動型企業 OS の個人版を $0 で構築できる唯一の規律 OS。
...
| 2 | 6 部門 27 エージェント整備 | 完了 | コンサル → 実装 → マーケ → 海外まで一気通貫 |
...
- [anthropics/skill-creator](https://github.com/anthropics/skills/tree/main/skills/skill-creator): SKILL.md 設計の本家ガイド、33 スキル拡張時の作法統一に活用
