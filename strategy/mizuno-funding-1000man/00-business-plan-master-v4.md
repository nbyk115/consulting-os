...
| バージョン | v4（v3 = 銀行審査部 / 投資助言業前提、ハルシネーション訂正で全面書き直し）|
| 作成日 | 2026-05-08 |
...
### 削除済み旧 v3 属性（PR BM 訂正、参照不可）

- 「Pivot Markets / 三菱 UFJ 系（INFERENCE）」← ハルシネーション、出典不在
- 「銀行審査部出身（想定）」← ハルシネーション起点の前提
- 「中堅 M&A / 投資助言業」← 旧フレーム、削除済
...
| 法務 + 契約 | 150 万円 | OEM 契約書ひな形 / 出資契約書（J-KISS / SAFE / 種類株）/ 知財・NDA 整備 | SPECULATION |
...
漸進 vs 大胆比率宣言 + 失敗コスト上限管理 + 罰則 / 報酬対称配置。

ConsultingOS 適用: 1000 万円 = 18 ヶ月の RTO（Race to Occupy）資金として失敗コスト上限を物理化（Huang「倒産 30 日前マインド」）。
...
1. 佐藤裕介: 構造で売る / 3 変数交点 / アセット帰属診断
2. 小野寺信行: CPC/CPA 脱却 / 60/40 ルール / 目的別 KPI 分岐
3. Huang: 思考深度の証明 / 倒産 30 日前マインド
4. 孫正義: 登りたい山 / 動くための計画
5. マスク式 4 原則: インセンティブ + 評判資本主義 + Unkind Truth + アルゴリズム
...
| バリュエーション | 1 億円キャップ候補 | INFERENCE（事業実績ゼロ段階の推定）|
...
| 優先 | 確認事項 | リスクレベル |
|---|---|---|
| 1 | J-KISS / 普通株の選択と契約書ドラフト（水野氏出資形態・バリュエーションキャップ設定）| 高 |
| 2 | 水野氏前職 NDA / 競業避止条項の射程確認（ジーニー退任後の活動制約）| 高 |
| 3 | AGPL-3.0 SaaS 提供時のソース開示義務範囲（Twenty CRM / SuiteCRM の N&Y Craft 向け OEM 提供）| 高 |
| 4 | Anthropic Usage Policy の再包装提供該当性確認（ConsultingOS SaaS 化）| 中 |
| 5 | N&Y Craft 顧客データの匿名化要件 + 他案件転用の個人情報保護法適法性 | 中 |
| 6 | 副業フレームにおける本業雇用契約との整合性確認（水野氏出資後の立場変化含む）| 低-中 |
...
- 旧 6 ファイル（00-business-plan-master.md / 01-pitch-case-a / 03-pitch-case-c / 04-mtg-playbook / 05-corrections-and-fact-check / case-a.html）の「銀行審査部 / Pivot Markets / 三菱 UFJ / 野村幸司」ハルシネーション grep 訂正は本コミットでは未実施（次コミット対応）
...
- 旧 6 ファイル（00-business-plan-master.md / 01-pitch-case-a.slides.md / 03-pitch-case-c.md / 04-mtg-playbook.md / 05-corrections-and-fact-check.md / case-a.html）の「銀行審査部 / Pivot Markets / 三菱 UFJ / 野村幸司」ハルシネーション grep 訂正
- HTML pitch v4 派生（case-a-v4.html / case-b-v4.html / case-c-v4.html / index-v4.html）
- demo-consultingos-financial.html を「マーケ Agent demo」へ書き換え or 削除判断
- 04-mtg-playbook-v4.md 想定 Q&A v4 更新
- oem-packaging-mizuno.md §5-B 金融投資家フレーム残存箇所の v4 整合更新
