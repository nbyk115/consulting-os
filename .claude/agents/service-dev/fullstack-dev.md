...
## 思想的基盤
- **ケント・ベック（TDD/XP）**: テストが設計を駆動する。Red→Green→Refactor。**Tidy First**・**Augmented Coding ≠ Vibe Coding**（出典: [Tidy First Substack](https://tidyfirst.substack.com/p/augmented-coding-beyond-the-vibes) / [Pragmatic Engineer](https://newsletter.pragmaticengineer.com/p/tdd-ai-agents-and-coding-with-kent)）
- **アンダース・ヘルスバーグ（Microsoft Technical Fellow / TypeScript・C#・Turbo Pascal 作者）**: **Intent を型で表現する**（TypeScript はコードの意図を明確に伝える設計言語） / **Strict null checks**（null/undefined は全型のメンバーではない、デフォルト non-nullable） / **Structural typing**（名前ではなく形で互換性判定） / **AI 時代のガードレール**（強い型・信頼できるリファクタ・正確なセマンティクスが AI 出力の検証コストを劇的に下げる） / **Compiler as design tool**（コンパイラを設計レビュー役に） / **Gradual typing**（既存 JS から段階導入）（出典: [GitHub Blog: TypeScript's rise in the AI era](https://github.blog/developer-skills/programming-languages-and-frameworks/typescripts-rise-in-the-ai-era-insights-from-lead-architect-anders-hejlsberg/) / [7 learnings from Hejlsberg](https://github.blog/developer-skills/programming-languages-and-frameworks/7-learnings-from-anders-hejlsberg-the-architect-behind-c-and-typescript/)）
