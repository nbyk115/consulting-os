...
#### 7. Demand Elegance（エレガンス要求 — Balanced）
> 出典: Workflow Orchestration ベストプラクティス

- **非自明な変更では一拍置く**: 「もっとエレガントな方法はないか？」と自問してから手を動かす
- **ハッキーに感じたら再設計**: "Knowing everything I know now, implement the elegant solution" — 全知識前提で書き直す
- **自明な修正には適用しない**: タイポ修正・1行 fix では over-engineering を避ける（外科的変更の原則と整合）
- **発表前に自己挑戦**: 「これをスタッフエンジニア／佐藤裕介がレビューしたら通るか？」と問う

#### 8. Verification Before Done（完了宣言の前に証明）
> 反証モードと連動。「動くはず」では完了にしない。

- **証明なしで complete マークしない**: テスト・ログ・差分で動作を実証する
- **main との差分を比較**: 関連ある変更なら behavior diff を取る
- **メンタルチェック**: 「スタッフエンジニアがこれを承認するか？」を出力前に自問
- ConsultingOS では反証モード Step 3（実用反証）がこれに相当

#### 9. Autonomous Bug Fixing（手取り足取り不要の自律修正）
- **バグ報告 = 修正開始**: ユーザーに「どう直しますか？」と聞き返さない
- **ログ→失敗テスト→根本原因→修正** を一気通貫で完走
- **CI 失敗も同様**: 失敗ログを指されたら、指示なく fix まで進める
- **例外**: アーキテクチャ判断を伴う場合は Plan Mode で承認を取る（規律 #1 と整合）
