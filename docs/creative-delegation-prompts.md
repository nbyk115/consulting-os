...
> クリエイティブチームの各 agent が skill / デザイン参照 / 作業フローをきちんと使うための完成形委任プロンプト。orchestrator はデザインタスクで agent を起動する時、本テンプレートをコピーして使う。雑な委任 (skill / ツール未明示) が「クリエイティブチームがツールを使わない」根本原因のため、本テンプレートで委任を標準化する。

## 共通ルール (全テンプレート適用済)

各テンプレートには以下が組み込み済:
- skill 明示参照 (PR #219/#230、skill-reference-check.sh が未明示を warn)
- ツール選定 (成果物生成は HTML+CSS 直接制作が標準。写真的画像は外部モデル + claude-design-handoff の 3 点セット protocol。Canva MCP は本環境で生成画像取り込み不可のため納品物生成に使わない、2026-05-18 改訂)
...
手順:
1. デザイン参照を取得 (WebSearch / Lazyweb For Humans / Mobbin、3-5 件。URL と構造を言語化)
2. competitive-analyst に競合比較を依頼、「業界標準の型」と「断つべき平均値」を明文化
3. 成果物生成のツール選定: HTML+CSS 直接制作が標準。写真的画像が要る場合は claude-design-handoff の「外部画像生成オーケストレーション protocol」で 3 点セットをサジェスト (DESIGN.md §12.3)
...
2. デザイン成果物は HTML + CSS 直接制作 (sales-deck-designer 実装、Chromium レンダリング)。テキスト主体は Google Slides 可
...
YOU MUST: orchestrator は creative 部門の agent を起動する時、本テンプレートの該当 agent 版をコピーし [案件名] [成果物] 等を埋めて委任する。テンプレートを使わない雑な委任 (skill / ツール / フロー未明示) は禁止。skill-reference-check.sh hook が skill 未明示を warn する。

## 反証チェック (Step 1-4 圧縮)

- Step 1: creative 部門 6 agent FACT (.claude/agents/creative/) / 各 agent の skill 対応 FACT (creative-playbook / distinctiveness / document-creation-6-steps 等)
- Step 2: PR #219/#230 (skill 明示) + PR #227 (視線フロー) + PR #226 (識別性ゲート) + PR #213 (2 段階検証) を各テンプレートに統合、規律でなく実用委任テンプレート (Hard Rule 13 非該当)。2026-05-18 改訂: Canva MCP 第一選択 (旧 PR #235) は本環境で生成画像取り込み不可のため撤回、HTML+CSS 直接制作 + 外部画像 protocol に差し替え
- Step 3 実用反証: 「クリエイティブチームがツールを使わない」根本原因 = 委任が雑で skill/ツール未明示、を完成形テンプレートで構造解決
- Step 4 リスク即潰し: 雑な委任 → 本テンプレートをコピーで使う運用 + skill-reference-check.sh / テンプレート形骸化 → creative-director.md から本ファイル参照、各案件で実使用
