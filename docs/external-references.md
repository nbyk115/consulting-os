...
| Business Insider「トップ SaaS の 2/3 は生き残れない、AI 時代に」（アナリスト分析、HubSpot / Atlassian / Figma 下落）| [businessinsider.com](https://www.businessinsider.com/) | 採用（ConsultingOS SaaS 崩壊耐性訴求） | AI エージェントが発注 + プロダクト作り → UI 不要 + 人不要、エージェントは UI を飛ばしてデータ直接操作、人間がワークフローから外れる → ユーザー課金モデル崩壊、生き残るのは「データ + ワークフロー深層食い込み」企業 = Palantir 型 / ConsultingOS 型、ConsultingOS は SaaS ではなく OS（Claude Code 拡張型）= SaaS 崩壊論の影響圏外 + 業界変革を加速する側、monopoly-positioning §10-B-2 死の谷課題マッピングへの追加候補（SaaS 依存度軸）、market-researcher + competitive-analyst + strategy-lead で活用 |
